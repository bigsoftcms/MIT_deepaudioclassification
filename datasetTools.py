...
from config import datasetPath_predict
...
# Loads dataset mode "predict"
def getDataset_predict(nbPerGenre, genres, sliceSize, mode="predict"):
    print("[+] Dataset name: {}".format(getDatasetName(nbPerGenre,sliceSize)))
    createDatasetFromSlices_predict(nbPerGenre, genres, sliceSize) 
    return loadDataset_predict(nbPerGenre, genres, sliceSize, mode)
...
def loadDataset_predict(nbPerGenre, genres, sliceSize, mode="predict"):
    #Load existing
    datasetName = getDatasetName(nbPerGenre, sliceSize)
    print("[+] Loading datasets for prediction... ")
    train_X = pickle.load(open("{}train_X_{}.p".format(datasetPath_predict,datasetName), "rb" ))
    train_y = pickle.load(open("{}train_y_{}.p".format(datasetPath_predict,datasetName), "rb" ))
    print("    Datasets for Prediction loaded! ✅")
...
#Saves dataset
def saveDataset_predict(train_X, train_y, nbPerGenre, sliceSize):
     #Create path for dataset if not existing
    if not os.path.exists(os.path.dirname(datasetPath_predict)):
        try:
            os.makedirs(os.path.dirname(datasetPath_predict))
        except OSError as exc: # Guard against race condition
            if exc.errno != errno.EEXIST:
                raise
    ...
    #SaveDataset
    datasetName = getDatasetName(nbPerGenre, sliceSize)
    pickle.dump(train_X, open("{}train_X_{}.p".format(datasetPath_predict,datasetName), "wb" ))
    pickle.dump(train_y, open("{}train_y_{}.p".format(datasetPath_predict,datasetName), "wb" ))
    print("    Dataset saved! ✅💾")
...
#Creates and save dataset from slices
def createDatasetFromSlices_predict(nbPerGenre, genres, sliceSize):
    data = []
    genre = "predict"
    #Get slices in genre subfolder
    filenames = os.listdir(slicesPath_predict+genre)
    filenames = [filename for filename in filenames if filename.endswith('.png')]
    filenames = filenames[:nbPerGenre]
    #Randomize file selection for this genre
    shuffle(filenames)
    ...
    #Add data (X,y)
    for filename in filenames:
        imgData = getImageData(slicesPath_predict+genre+"/"+filename, sliceSize)
        label = 1
        data.append((imgData,label))
    ...
    #Save
    saveDataset_predict(train_X, train_y, nbPerGenre, sliceSize)
